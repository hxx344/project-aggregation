...
新项目可以只提供网页入口，也可以通过统一摘要接口加入工作台。摘要读取和 Asset 后台同步由工作台后端发起，不需要为浏览器开放跨域接口。原页面可选择经工作台代理或直接访问，仍保留原项目认证。
...
| `link` | 不采集摘要；`proxy` 时转发用户的原页面请求 | 原页面自己登录，保存的摘要凭据不会自动注入页面。 |
...
页面访问先由工作台会话签发一次性授权票据，再建立绑定该会话和项目主机的 host-only Cookie。退出工作台或修改项目配置会撤销页面访问。摘要密码及摘要会话 Cookie 仅留在服务器，页面代理不会自动注入保存的密码；用户需在原项目登录一次，之后遵循原项目会话规则。
