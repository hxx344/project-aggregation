...
- 项目入口：通过工作台打开完整原页面，或直接访问项目地址；原项目保持独立登录。
...
工作台的自动任务不会下单、启停策略或转账；Asset 同步只调用原项目已有的资产同步功能，会更新估值和历史记录。原页面的操作仍由原项目处理，工作台不会合并原项目的登录会话。
...
保存的密码和摘要会话 Cookie 只留在工作台服务器，用于摘要读取及 Asset 后台同步。页面代理不会自动注入保存的密码：首次打开原项目时，仍需在原页面登录一次。页面访问使用一次性授权票据和绑定工作台会话的独立 host-only Cookie；退出工作台或修改项目配置后，已有页面访问授权失效。

代理模式允许原页面嵌入工作台：代理将 `X-Frame-Options` 和 CSP 的 `frame-ancestors` 统一为仅允许当前工作台嵌入，保留其余 CSP 限制。也可以用“打开项目页面”独立操作。直接访问模式仍受原页面自身的嵌入策略限制。
...
| 资产更新时间 | 动态资产行更新时间中的最早值，明确标记 `mode=manual` 的手工行不参与实时过期计算；手工行最早估值记录时间单独展示。纯手工账本标记“静态估值”，保留估值记录时间。后台同步状态单独展示，不代替资产源时间。 |
...
通过工作台打开 Asset 原页面时，已开启后台同步且已保存密码的 `/api/sync` 请求与后台共用一次同步。代理先以浏览器原有登录验证账本访问权限，同步完成后再以同一浏览器登录读取真实账本，避免同时刷新造成锁冲突。未登录不能读取；同步失败不伪装为成功。关闭后台同步后，原页面同步请求照常转发。

## 接入后续项目

1. 只需要入口：在项目管理选择 `link`，默认 `direct`，填写浏览器可访问的页面地址。若需要通过工作台访问，可改为 `proxy` 并填写接口地址；链接模式不会采集摘要。
2. 需要摘要：选择 `standard`，让新项目实现 `GET /api/hub/summary`。可选 HTTP Basic 认证，填写用户名与密码；页面默认 `direct`，也可改为 `proxy`。
