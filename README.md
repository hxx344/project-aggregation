...
- 工作台后端每 30 秒读取已启用项目的现有数据，工作台页面关闭后仍会读取。读取不会触发 Asset 同步，也不会代替原项目的行情采集。
...
```bash
ssh -N -o ExitOnForwardFailure=yes -L 3100:127.0.0.1:3100 -L 3000:127.0.0.1:3000 -L 5678:127.0.0.1:5678 -L 18765:127.0.0.1:8765 user@server
```

保持这个终端运行，四个默认页面地址即可从电脑访问。ASTER 的电脑端口是 `18765`，转发到服务器的 `8765`。如果某个本地端口被占用，可修改对应 `-L` 的第一个端口，同时修改工作台里的“页面地址”；“接口地址”继续填写服务器内的原端口。

只保留 `3100` 转发也能打开工作台；后端与三个项目在同一台服务器，会直接读取 `3000`、`5678`、`8765`，不依赖电脑上的原项目转发。浏览器要打开或嵌入三个完整原页面，仍需对应端口转发或可访问的域名。
...
| --- | --- | --- | --- |
| ASTER 5X | `http://127.0.0.1:18765/` | `http://127.0.0.1:8765` | `aster` |
...
升级不会自动改写已经保存的项目配置。若现有 ASTER 接口地址仍是 `http://127.0.0.1:18765`，请在“项目管理”中将接口地址改为 `http://127.0.0.1:8765`，重新填写 ASTER 网页登录密码并保存；页面地址仍可保留 `http://127.0.0.1:18765/`。
...
Asset 当前由原页面在可见时每 60 秒触发同步；工作台只执行 `GET /api/ledger` 读取已保存的账本，不会触发同步。原页面关闭或不可见后，这个自动同步周期停止，动态资产和汇率会随源数据时间变旧而显示过期。因此，只打开工作台并保留 `3100` 转发，不能让 Asset 源数据持续自动更新；需要通过对应转发打开 Asset 原页面，完成并保持原项目同步。
