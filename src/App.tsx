...
function OverviewPage({ snapshots, onProject, onEdit, onManage, onPreload }: { onPreload: (id: string) => void; snapshots: Snapshot[]; onProject: (id: string) => void; onEdit: (p: Project) => void; onManage: () => void }) {
  ...
  return <>
    ...
    <section className="projects-overview"><div className="section-heading"><h2>项目工作区</h2><button className="text-button" onClick={onManage}>管理项目<ChevronRight size={16} /></button></div><div className="project-grid">{snapshots.map(s => <ProjectCard key={s.project.id} snapshot={s} onPreload={() => onPreload(s.project.id)} onOpen={() => onProject(s.project.id)} onEdit={() => onEdit(s.project)} />)}</div>{!snapshots.length ? <div className="empty-state"><FolderKanban /><p>添加项目后，概览会出现在这里。</p><button className="button secondary" onClick={onManage}>添加项目</button></div> : null}</section>
  </>;
}
...
function ProjectCard({ snapshot: s, onOpen, onEdit, onPreload }: { onPreload: () => void; snapshot: Snapshot; onOpen: () => void; onEdit: () => void }) {
  return <article className="project-card" onPointerEnter={onPreload} onFocusCapture={onPreload}><div className="project-card-top"><span className={`project-icon ${s.project.category}`}><ProjectIcon category={s.project.category} /></span><Status state={s.state} adapter={s.project.adapter} /></div><h3>{s.project.name}</h3><p className="project-description">{s.project.description || categories[s.project.category]}</p>{s.metrics.length ? <div className="mini-metrics">{s.metrics.slice(0, 2).map(m => <MetricView metric={m} key={m.key} />)}</div> : <div className="project-empty"><span>暂未取得概览数据</span><small>{s.message || '配置数据连接后自动读取'}</small></div>}<div className="project-time">{s.freshness === 'static' ? '手工估值记录' : '源数据'} {formatDate(s.updatedAt, true)}</div><div className="project-card-actions"><button className="button secondary" onClick={onOpen}>进入项目<ArrowUpRight size={15} /></button><button className="icon-button" aria-label={`配置${s.project.name}`} title="配置连接" onClick={onEdit}><Settings2 size={17} /></button></div></article>;
}
