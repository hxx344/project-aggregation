...
function ProjectPage({ snapshot: s, onEdit, onCheck, checking }: { snapshot: Snapshot; onEdit: () => void; onCheck: () => void; checking: boolean }) {
  ...
  const proxied = s.project.accessMode === 'proxy';
  const accessible = s.project.enabled && !!(proxied ? s.project.apiUrl : s.project.url);
  useEffect(() => { setEmbedSrc(''); setAccessError(''); setSyncRequested(false); }, [s.project.id, s.project.apiUrl, s.project.accessMode, s.project.mode, s.project.enabled]);
  async function pageUrl() {
    if (!proxied) return s.project.url;
    const result = await api<{ url: string }>(`/api/projects/${encodeURIComponent(s.project.id)}/launch`, { method: 'POST' });
    return result.url;
  }
  async function openPage(embedded = false) {
    setAccessError('');
    const tab = embedded ? null : window.open('about:blank', '_blank');
    if (!embedded && !tab) { setAccessError('浏览器阻止了新窗口，请允许此页面打开新窗口后重试。'); return; }
    if (tab) { tab.opener = null; tab.document.title = '正在打开项目'; tab.document.body.textContent = '正在打开' + s.project.name + '…'; }
    setOpening(true);
    try { const url = await pageUrl(); if (embedded) setEmbedSrc(url); else if (tab) tab.location.replace(url); }
    catch (error) { tab?.close(); setAccessError(error instanceof Error ? error.message : '项目页面暂时无法打开'); }
    finally { setOpening(false); }
  }
  async function syncNow() {
    setAccessError(''); setSyncRequested(true);
    try { await api(`/api/projects/${encodeURIComponent(s.project.id)}/sync`, { method: 'POST' }); onCheck(); }
    catch (error) { setAccessError(error instanceof Error ? error.message : '同步请求未完成'); }
    finally { setSyncRequested(false); }
  }
  return <>
    <div className="project-toolbar"><div><Status state={s.state} adapter={s.project.adapter} /><span className="muted">检查于 {formatDate(s.checkedAt, true)}</span></div><div><Button className="button secondary" onClick={onCheck} busy={checking} disabled={!s.project.enabled}><RefreshCw size={16} />检查连接</Button><button className="button secondary" onClick={onEdit}><Settings2 size={16} />配置连接</button>{accessible ? <Button className="button primary" busy={opening} onClick={() => void openPage()}><ExternalLink size={16} />打开项目页面</Button> : null}</div></div>
    {accessError ? <div className="error-box" role="alert">{accessError}</div> : null}
    {s.project.adapter === 'asset' ? <section className="panel sync-panel" aria-label="资产后台同步"><div><strong>{s.project.autoSync ? '后台同步已开启' : '后台同步已关闭'}</strong><p>{s.sync?.message || (s.project.autoSync ? '服务器每分钟同步资产，关闭页面后仍会更新。' : '保留现有账本，停止定时更新。')}</p><small>最近完整同步：{formatDate(s.sync?.lastSuccessAt || null, true)} · 各资产仍保留自己的源时间</small></div>{s.project.autoSync ? <Button className="button secondary" busy={syncRequested || s.sync?.state === 'syncing'} disabled={!s.project.enabled || !s.project.hasCredentials} onClick={() => void syncNow()}><RefreshCw size={16} />同步资产</Button> : <button className="button secondary" onClick={onEdit}>修改设置</button>}</section> : null}
    ...
    <section className="panel details-panel"><div className="panel-heading"><h2>项目概览</h2><span className="muted">{s.freshness === 'static' ? '手工估值记录于' : '源数据更新于'} {formatDate(s.updatedAt, true)}</span></div>{s.metrics.length ? <div className="detail-metrics">{s.metrics.map(m => <MetricView key={m.key} metric={m} />)}</div> : <div className="empty-state"><ProjectIcon category={s.project.category} size={34} /><h3>{s.project.adapter === 'link' ? '此项目作为网页入口接入' : '等待项目数据'}</h3><p>{s.project.adapter === 'link' ? '通过“打开项目页面”访问完整功能。' : '检查连接后，已保存的概览指标会出现在这里。'}</p><button className="button secondary" onClick={onEdit}>配置连接</button></div>}{s.trend && s.trend.length > 1 ? <div className="detail-chart"><h3>历史记录</h3><ChartBoundary><Suspense fallback={<p>正在载入曲线</p>}><TrendChart points={s.trend} label={s.project.adapter === 'asset' ? '表内总额' : '记录值'} unit={s.project.adapter === 'asset' ? 'USD' : undefined} /></Suspense></ChartBoundary></div> : null}</section>
    {s.project.mode === 'embed' && accessible ? <section className="panel embedded-panel"><div className="panel-heading"><h2>完整页面</h2><Button className="button ghost" busy={opening} onClick={() => void openPage()}><ExternalLink size={15} />单独打开</Button></div><p className="embed-note">{proxied ? (s.project.hasCredentials && s.project.adapter !== 'link' ? '通过工作台自动登录，无需额外 SSH 隧道或重复输入项目密码。' : '无需额外 SSH 隧道。在连接设置中保存项目登录信息后，可自动登录支持的项目。') : '原页面可能限制嵌入；无法载入时请单独打开。'}</p>{embedSrc ? <><iframe title={s.project.name + '完整页面'} src={embedSrc} referrerPolicy="no-referrer" sandbox="allow-scripts allow-same-origin allow-forms allow-popups allow-downloads" /><Button className="button ghost" busy={opening} onClick={() => void openPage(true)}>重新载入页面</Button></> : <div className="embed-start"><Button className="button secondary" busy={opening} onClick={() => void openPage(true)}>载入项目页面</Button><span>按需载入，后台资产同步独立运行。</span></div>}</section> : null}
  </>;
}
